...
Настоящая разница между "9B не справляется" и "9B справился за 3 шага" — это не размер весов и не квант. Это наличие цикла обратной связи. Модель, которая видит результат своей работы и может его исправить, стабильно обгоняет модель, которая просто генерирует ответ и надеется на лучшее.

Из того что получилось и что удивило:
- Переименование инструментов дало заметный прирост точности без изменения промптов
- Маскировка старых выводов работает лучше суммаризации — и это контринтуитивно
- Inverse scaling для SLM реален: "думай дольше" иногда ухудшает результат
- 9B с 8-bit и правильным harness'ом практически всегда выигрывает у 30B с iq2_xxs

Что не работает и пока не решено: большие кодовые базы (500+ файлов), задачи без явного verifier, всё что требует глобального понимания архитектуры. Это честные ограничения, не маркетинг.

lema — опенсорсный проект, MIT, TypeScript, zero runtime deps. Если хочется потрогать самому:
...
Исходники и issues: [github.com/iivgll/lema](https://github.com/iivgll/lema)

Если тестировали похожие подходы или есть мысли по архитектуре — интересно услышать в комментариях.
