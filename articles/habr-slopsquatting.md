...
На прошлой неделе я попросил Claude Code добавить рендеринг markdown в проект. Через секунду в зависимостях появился [`flutter_markdown`](https://pub.dev/packages/flutter_markdown), нормальный пакет от `flutter.dev`. Я нажал accept. А потом случайно открыл его карточку, а там прямо в шапке:

> This project has been discontinued, and will not receive further updates.

Пакет уже больше года как мёртв, есть преемник: [`flutter_markdown_plus`](https://pub.dev/packages/flutter_markdown_plus). Просто в обучающих данных агента он всё ещё живой.

Другой пример, ещё свежее. Скажешь Cursor «нужно поднять S3-совместимое хранилище для тестов», и он напишет docker-compose с `minio/minio:latest`. Только вот [**репозиторий `minio/minio` архивирован 25 апреля 2026 года**](https://github.com/minio/minio), MinIO Inc. толкает всех в коммерческий AIStor ([подробнее](https://itsfoss.com/news/minio-moves-away-from-open-source/)). SDK ещё обновляется, но сам сервер мёртв.
...
<!-- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🖼  КАРТИНКА #2 — ДВА СКРИНШОТА РЯДОМ (КОЛЛАЖ)

Самое сильное здесь — настоящие скриншоты, а не иллюстрация.
Композиция: один кадр, разбитый пополам вертикальной линией.

ЛЕВАЯ ПОЛОВИНА — скриншот страницы pub.dev/packages/flutter_markdown
с явно видимой жёлто-красной плашкой:
"This project has been discontinued, and will not receive further updates."
Заголовок пакета и плашка должны быть в фокусе, остальное можно
слегка затемнить или обрезать.

ПРАВАЯ ПОЛОВИНА — скриншот github.com/minio/minio с серым баннером:
"This repository was archived by the owner on Apr 25, 2026. It is
now read-only." + строкой "Public archive" под названием репозитория.

Между половинами — тонкая разделительная линия, сверху одной общей
полосой подпись: "ОБА ПАКЕТА ЕЩЁ ВЧЕРА БЫЛИ СТАНДАРТОМ".

Стиль: минималистичный коллаж, без рамок и тени. Подпись —
крупный sans-serif, тревожный красный или жёлтый.

Как сделать: открыть оба сайта в браузере, сделать скриншоты
(Cmd+Shift+4 на macOS / Win+Shift+S), скомпоновать в Figma /
Photopea / любом редакторе. На каждой плашке можно дорисовать
красную обводку маркером для акцента.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ -->
...
Год назад я полтора дня дебажил неприятный баг. Flutter-приложение, авторизация через [`flutter_secure_storage`](https://pub.dev/packages/flutter_secure_storage). Де-факто стандарт хранения токенов в Keychain и Android Keystore. Активный мейнтейнер, миллионы скачиваний.
...
А когда я открыл GitHub Issues самого пакета, там был ад. [#284](https://github.com/juliansteenbakker/flutter_secure_storage/issues/284): «данные пропадают после обновления через App Store». [#223](https://github.com/juliansteenbakker/flutter_secure_storage/issues/223): «после 2+ часов в background». [#524](https://github.com/juliansteenbakker/flutter_secure_storage/issues/524): «массово на iOS 16.3+». Сотни комментариев, годами. Корневая причина: `KeychainAccessibility` применялся только при `SecItemAdd`, а на update и read не применялся. Починили в v9.1.0.
...
Главная работа здесь: **«We Have a Package for You!»** ([arXiv:2406.10279](https://arxiv.org/abs/2406.10279), USENIX Security 2025). 576 000 примеров кода, 16 моделей:
...
- [**ultralytics**](https://blog.pypi.org/posts/2024-12-11-ultralytics-attack-analysis/) (PyPI, декабрь 2024): XMRig-майнер через GitHub Actions injection, пакет с 60 млн скачиваний.
- [**@lottiefiles/lottie-player**](https://thehackernews.com/2024/10/lottiefiles-issues-warning-about.html) (октябрь 2024): фишинг мейнтейнера, crypto-drainer в браузере.
- **eslint-config-prettier** (июль 2025, CVE-2025-54313): фишинг с домена `npnjs.com`, скомпрометированы пакеты с 78 млн weekly downloads.
- [**Shai-Hulud**](https://www.cisa.gov/news-events/alerts/2025/09/23/widespread-supply-chain-compromise-impacting-npm-ecosystem) (сентябрь 2025) → **2.0** (ноябрь 2025): первый self-replicating npm-червь, во второй волне 25 000+ malicious-репозиториев.
...
---
name: check-dep
description: Проверить пакет перед добавлением. Используй каждый раз, когда добавляешь новую зависимость в package.json, pubspec.yaml, requirements.txt, pyproject.toml, go.mod или Cargo.toml. Также по команде /check-dep <имя>.
---

# Проверка пакета перед установкой

Перед любым `npm install`, `flutter pub add`, `pip install` или эквивалентом — пройди проверку. Хотя бы один красный пункт — остановись и спроси пользователя.

## Шаги

1. **Реестр** по контексту: npm → registry.npmjs.org, Flutter/Dart → pub.dev, Python → pypi.org, Go → pkg.go.dev, Rust → crates.io.
2. **Карточка пакета через WebFetch.** Статус (discontinued/deprecated/archived), дата последнего релиза, репо, паблишер, weekly downloads.
3. **Репозиторий через WebFetch.** Archived, дата последнего коммита, открытые issues, SECURITY.md, активность мейнтейнера.
4. **Advisory:** для npm — `npm view <name>` + osv.dev, для PyPI — osv.dev, для pub.dev — security advisories на карточке.
5. **Имя.** Похоже на популярный пакет, но не совпадает (`flutter-secure-storage` vs `flutter_secure_storage`, `requets` vs `requests`) — красный флаг.

## Отчёт

```
Пакет: <name>@<version>
Реестр: <npm|pypi|pub.dev|...>
Статус: [GREEN | YELLOW | RED]

— Поддержка: <active | maintenance | discontinued | archived>
— Последний релиз: <date>
— Последний коммит: <date>
— Архивирован: <yes | no>
— Issues open/closed: <X / Y>
— Advisories: <none | список CVE>
— Похожие имена: <none | список>

Рекомендация: <ставить | спросить | не ставить, альтернатива X>
```

## Светофор

- **RED**: discontinued, archived, последний релиз > 12 мес, critical-CVE, имя похоже на популярный пакет.
- **YELLOW**: maintenance mode, релиз 6–12 мес назад, > 50 открытых issues без ответа, нет SECURITY.md, weekly downloads < 1000.
- **GREEN**: всё остальное.

## Если не GREEN

Не ставь сам. Покажи отчёт и предложи альтернативу. Альтернативу ищи в поле "Replacement" на карточке, в README основного пакета, в топ-5 категории в реестре.
````
